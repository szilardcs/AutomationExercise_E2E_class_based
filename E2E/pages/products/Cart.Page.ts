...
export class Cart extends BasePage {
	...
	private rowSelector(productName: string): Locator {
		return this.cartWrapper.getByRole("row").filter({ hasText: `${productName}` });
	}
	...
	async verifyProductQuantity(productName: string, expectedQuantity: number): Promise<number> {
		const productWrapper = this.rowSelector(productName);
	...
	async verifyProductTotal(productName: string, productPrice: number, productQuantity: number): Promise<void> {
	...
	async verifyProductInCart(productName: string): Promise<void> {
		const productNameInCart = await this.getProductName(productName);
		expect(productNameInCart).toBeTruthy();
	}
